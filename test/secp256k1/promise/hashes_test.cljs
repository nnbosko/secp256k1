...
(deftest sha256-test
    (testing "Can asynchronously perform a SHA256 hash (empty input)"
      (async done
        (-> (promise-hashes/sha256 [])
          (.then (fn [out]
                   ...
                   (is (= (vec (sync-hashes/sha256))
                     ...
                     "No input")
                   (done))))))
    (testing "Can asynchronously perform a SHA256 hash ([1 2 3 4])"
      (async done
        (-> (promise-hashes/sha256 #js [1 2 3 4])
          (.then (fn [out]
                   (is (= (vec (sync-hashes/sha256 [1 2 3 4]))
                         (vec out)))
                   (done))))))
    (testing "Can asynchronously perform a SHA256 hash (\"foo\")"
      (async done
        (-> (promise-hashes/sha256 "foo")
          (.then (fn [out]
                   (is (= (vec (sync-hashes/sha256 "foo"))
                         (vec out)))
                   (done))))))
    (testing "Can asynchronously perform a SHA256 hash (\"مولوی\")"
      (async done
